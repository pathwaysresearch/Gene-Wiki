...
(Acemoglu and Restrepo, 2020; Boustan et al., 2022; Kanazawa et al., 2022). Insofar as
...
[Page 3]
different skill profiles? Do they affect workers' enjoyment of their work (Schwabe and
Castellacci, 2020)?

This paper takes the first step towards answering these questions.¹ In an online experiment,
...
In the treatment group, 92% of treated participants successfully sign up for ChatGPT,<sup>2</sup> and
...
<sup>2</sup>Our survey was mostly active only after 5pm EST, when ChatGPT was almost always up, ensuring a very
...
[CHART: A figure titled "Figure 1: Treatment Effects on Productivity" containing four subplots.
(a) "Time Taken Decreases" is a line plot showing Self-Reported Time Spent (Minutes) on the y-axis and Pre-Treatment/Post-Treatment on the x-axis. The "Treated" group's time drops from about 28 to 17 minutes, while the "Control" group's time remains stable around 27 minutes. An annotation reads "Treatment Effect: -0.83 SDs, 95% CI: [-0.63, -1.03]".
(b) "Average Grades Increase" is a line plot showing Mean Grade on the y-axis and Pre-Treatment/Post-Treatment on the x-axis. The "Treated" group's grade increases from about 4.0 to 4.5, while the "Control" group's grade remains stable around 3.8. An annotation reads "Treatment Effect: 0.45 SDs, 95% CI: [0.27, 0.63]".
(c) "Time Distribution (Second Task)" is a bar chart showing the Percent of Respondents on the y-axis and Minutes Spent on Second Task (in bins) on the x-axis. The "Treatment" group (mean 17) has a distribution skewed towards shorter times, peaking at 11-15 minutes. The "Control" group (mean 27) has a wider distribution, peaking around 26-30 minutes.
(d) "Grades (Second Task)" is a bar chart showing the Percent of Essay-Grades on the y-axis and Second-Task Grade (1-7) on the x-axis. The "Treatment" group (mean 4.54) has a distribution shifted towards higher grades compared to the "Control" group (mean 3.789).]
...
[CHART: This is a figure with two panels.
Panel (a) is titled "Grade Inequality Decreases". It is a scatter plot with "Mean Grade: Task 1" on the x-axis (from 1 to 7) and "Mean Grade: Task 2" on the y-axis (from 2 to 6). It shows two sets of data points with trend lines: hollow squares for the Control group and solid triangles for the Treatment group. The trend line for the Control group has a steeper positive slope than the trend line for the Treatment group. A text box inside the plot reads "Change in Slope: -0.243 95% CI: [-0.08, -0.41]". The legend below the chart provides the slopes and standard errors for each group.
Panel (b) is titled "Time Taken Decreases Across Grade Distribution". It is a scatter plot with "Mean Grade: Task 1" on the x-axis (from 1 to 7) and "Time Taken: Task 2" on the y-axis (from 10 to 40). It shows two sets of data points with trend lines: hollow squares for the Control group and solid triangles for the Treatment group. The trend line for the Control group has a slight positive slope, while the trend line for the Treatment group has a slight negative slope. The legend below the chart provides the slopes and standard errors for each group.]
...
[CHART: A bar chart titled "(a) Effects on Task Structure". The y-axis is labeled "Percent of Time Spent" and ranges from 0 to 60. The x-axis has three categories: "Brainstorming", "Rough-Drafting", and "Editing". Each category has two bars: a gray bar for "Control" and a teal bar for "Treatment".
- For "Brainstorming", the Control bar is at approximately 25% and the Treatment bar is at approximately 20%.
- For "Rough-Drafting", the Control bar is at approximately 55% and the Treatment bar is at approximately 30%.
- For "Editing", the Control bar is at approximately 25% and the Treatment bar is at approximately 40%.
All bars have error bars.]

(b) No Clear Heterogeneity in Benefits of ChatGPT by Relative Writing Skills

[CHART: This panel contains two sets of bar charts.
The top chart is titled "Outcome: Willingness to Pay for ChatGPT (% Salary)". The y-axis ranges from 0.2 to 0.8. The x-axis is divided into two main groups: "Self-Rated Communication Skills" and "Overall versus Writing Grade on First Task". Each group is further divided into "Bad Writer", "Medium", and "Good Writer". Each of these sub-categories has a gray "Control" bar and a teal "Treatment" bar. The values for all bars are between 0.3 and 0.5. The bars show a relatively flat trend across the different writer skill levels.
The bottom chart is titled "Outcome: Grade Gain from ChatGPT". The y-axis ranges from 0 to 1.5. The x-axis categories are the same as the top chart. Each sub-category has a gray "Control" bar and a teal "Treatment" bar. The values for all bars are between 0.6 and 0.8. The bars show a relatively flat trend across the different writer skill levels.
A legend at the bottom indicates the color coding for the x-axis sub-categories: a dark gray box for "Bad Writer", a medium teal box for "Medium", and a light teal box for "Good Writer".]
...
[CHART: A figure containing three charts labeled (a), (b), and (c).
Chart (a) is a line chart titled "Job Satisfaction". The y-axis is "Job Satisfaction (SDs)" ranging from -0.4 to 0.4. The x-axis shows "Pre-Treatment" and "Post-Treatment". Two lines are plotted: "Treated" (solid line with solid squares) and "Control" (dashed line with hollow squares). Both lines start near 0 at Pre-Treatment. At Post-Treatment, the Treated line increases to approximately 0.25, while the Control line decreases to approximately -0.25. Error bars are shown for each point. Text in the chart reads: "Treatment Effect: 0.5 SDs" and "95% CI: [0.32, 0.68]".
Chart (b) is a line chart titled "Self-Efficacy". The y-axis is "Self-Efficacy (SDs)" ranging from -0.2 to 0.2. The x-axis shows "Pre-Treatment" and "Post-Treatment". Two lines are plotted: "Treated" (solid line with solid squares) and "Control" (dashed line with hollow squares). Both lines start near 0 at Pre-Treatment. At Post-Treatment, the Treated line increases to approximately 0.1, while the Control line decreases to approximately -0.1. Error bars are shown for each point. Text in the chart reads: "Treatment Effect: 0.2 SDs" and "95% CI: [-0.02, 0.42]".
Chart (c) is a bar chart titled "Automation Beliefs". The y-axis is "Average Response (1-10 Scale)" ranging from 4 to 8. The x-axis has three categories: "Worried About Replacement", "Excited About Enhancement", and "Overall Optimism". For each category, there are two bars: a grey "Control" bar and a teal "Treatment" bar.
- For "Worried About Replacement", the Control bar is lower than the Treatment bar. Above this pair, text reads: "diff: 0.264 SDs" and "p-value: 0.006".
- For "Excited About Enhancement", the Control bar is lower than the Treatment bar. Above this pair, text reads: "diff: 0.389 SDs" and "p-value: 0.000".
- For "Overall Optimism", the Control bar is lower than the Treatment bar. Above this pair, text reads: "diff: 0.2 SDs" and "p-value: 0.037".
Error bars are shown on top of each bar.]
...
Kleinberg, Jon, Himabindu Lakkaraju, Jure Leskovec, Jens Ludwig, and Sendhil Mul-lainathan, "Human Decisions and Machine Predictions,” *Quarterly Journal of Economics*, 2018, 133 (1), 237–293.
